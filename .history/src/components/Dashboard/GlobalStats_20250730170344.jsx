...
export default function GlobalStats({ eventos }) {
  ...
  eventos.forEach((e) => {
    ...
    // ✅ Contar razones SOLO si es un evento PMA (edificios)
    if (
      e.evento &&
      e.evento.toLowerCase().includes("puerta mantenida") &&
      e["razones-pma"]
    ) {
      reasonCount[e["razones-pma"]] =
        (reasonCount[e["razones-pma"]] || 0) + 1;
    }
  ...
  const ubicacionMasFrecuente =
    ...
    ["Sin datos", 0];
  const razonMasFrecuente =
    Object.entries(reasonCount).sort((a, b) => b[1] - a[1])[0] ||
    ["Sin datos", 0];
  ...
  const diaPct =
    totalEventos > 0 ? Math.round((totalDia / totalEventos) * 100) : 0;
  const nochePct =
    totalEventos > 0 ? Math.round((totalNoche / totalEventos) * 100) : 0;
      ...
      <div className="global-grid">
        ...
        <div className="global-item">
          ...
          <p className="sub-info">
            ...
            <strong>{eventoMasFrecuente[0]}</strong> ({eventoMasFrecuente[1]})
          </p>
        </div>

   

        {/* Razón PMA más frecuente (solo edificios) */}
        <div className="global-item">
          <div className="icon-box">🔑</div>
          <p className="global-label">Razón PMA más frecuente</p>
          <h2 className="global-value">
            {razonMasFrecuente[0] !== "Sin datos"
              ? razonMasFrecuente[0]
              : "Sin datos"}
          </h2>
          <p className="sub-info">
