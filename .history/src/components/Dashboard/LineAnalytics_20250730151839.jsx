...
export default function LineAnalytics({ eventos, cliente }) {
  ...
  ultimosEventos.forEach((e) => {
    ...
    if (!grouped[key]) {
      grouped[key] = { total: 0, ingreso: 0, salida: 0, energia: 0, restauracion: 0, pma: 0, forzada: 0, alarmas: 0, confirmados: 0, falsos: 0, detalles: [] };
    }
    grouped[key].total++;
  ...
  if (cliente === "TGS") {
    datasets.push(
      { label: "Ingreso Personal", data: sortedKeys.map((k) => grouped[k].ingreso), borderColor: "#2563eb", backgroundColor: "rgba(37,99,235,0.2)", fill: true },
      { label: "Salida Personal", data: sortedKeys.map((k) => grouped[k].salida), borderColor: "#10b981", backgroundColor: "rgba(16,185,129,0.2)", fill: true },
      { label: "Corte Energía", data: sortedKeys.map((k) => grouped[k].energia), borderColor: "#f59e0b", backgroundColor: "rgba(245,158,11,0.2)", fill: true },
      { label: "Restauración Energía", data: sortedKeys.map((k) => grouped[k].restauracion), borderColor: "#eab308", backgroundColor: "rgba(234,179,8,0.2)", fill: true }
    );
  } else if (cliente === "Edificios") {
    datasets.push(
      { label: "PMA", data: sortedKeys.map((k) => grouped[k].pma), borderColor: "#f97316", backgroundColor: "rgba(249,115,22,0.2)", fill: true },
      { label: "Puerta Forzada", data: sortedKeys.map((k) => grouped[k].forzada), borderColor: "#dc2626", backgroundColor: "rgba(220,38,38,0.2)", fill: true },
      { label: "Alarmas", data: sortedKeys.map((k) => grouped[k].alarmas), borderColor: "#7c3aed", backgroundColor: "rgba(124,58,237,0.2)", fill: true }
    );
  } else {
    // Fallback general
    datasets.push({
