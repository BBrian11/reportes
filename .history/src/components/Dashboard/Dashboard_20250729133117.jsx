  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "novedades/edificios/eventos"), (snapshot) => {
      const data = snapshot.docs.map((doc) => {
        ...
        return {
          id: doc.id,
          cliente: "Edificios", // Forzamos categoría
          evento: d["evento-edificio"] || d["evento-tgs"] || d["evento-vtv"] || "Sin Evento",
          ubicacion: d["departamento"] || d["locaciones-tgs"] || d["planta-vtv"] || "Sin Ubicación",
          fecha: d.fechaHoraEnvio
            ? new Date(d.fechaHoraEnvio.seconds * 1000).toLocaleString("es-AR")
            : "Sin Fecha",
          observacion: d["observaciones-edificios"] || d["observaciones-tgs"] || "Sin Observación",
        };
      });
      setEventos(data);
    });
