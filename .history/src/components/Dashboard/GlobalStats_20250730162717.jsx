...
export default function GlobalStats({ eventos }) {
  ...
  const parseFecha = (valor) => {
    if (valor?.seconds) {
      return new Date(valor.seconds * 1000); // Firestore timestamp
    }
    if (typeof valor === "string") {
      // Extraer datos usando regex
      const match = valor
        .replace(/\u202F/g, " ") // remover espacios finos
        .match(/(\d{1,2}) de (\w+) de (\d{4}), (\d{1,2}):(\d{2}):(\d{2})/);

      if (match) {
        const [_, dia, mesTxt, anio, hora, min, seg] = match;
        const meses = [
          "enero", "febrero", "marzo", "abril", "mayo", "junio",
          "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"
        ];
        const mes = meses.indexOf(mesTxt.toLowerCase());
        return new Date(anio, mes, parseInt(dia), parseInt(hora), parseInt(min), parseInt(seg));
      }
    }
