  ...
  const handleSaveObservation = async () => {
    ...
    try {
      ...
      switch (selectedEvent.cliente) {
        case "TGS": path = `novedades/tgs/eventos/${selectedEvent.id}`; break;
        case "Edificios": path = `novedades/edificios/eventos/${selectedEvent.id}`; break;
        case "VTV": path = `novedades/vtv/eventos/${selectedEvent.id}`; break;
        case "Barrios": path = `novedades/barrios/eventos/${selectedEvent.id}`; break;
        default: path = `novedades/otros/eventos/${selectedEvent.id}`;
      }
      ...
      {selectedEvent && (
        <div className="modal-overlay">
          <div className="modal-card animate-fadeIn">
            <h3 className="modal-title">Editar Observación</h3>
            <p className="modal-info">
              <strong>Evento:</strong> {selectedEvent.evento} <br />
